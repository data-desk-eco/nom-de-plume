...
def parse_info_record(data: bytes) -> InfoRecord:
    ...
    try:
        ...
        effective_month = int(data[22:24].decode('cp500').strip() or '0')
        effective_day = int(data[24:26].decode('cp500').strip() or '0')
        approval_year = int(data[26:30].decode('cp500').strip() or '0')
        approval_month = int(data[30:32].decode('cp500').strip() or '0')
        approval_day = int(data[32:34].decode('cp500').strip() or '0')
