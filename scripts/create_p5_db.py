...
def main():
    ...
    org_csv = sys.argv[2] if len(sys.argv) > 2 else "data/p5_org.csv"
    specialty_csv = sys.argv[3] if len(sys.argv) > 3 else "data/p5_specialty.csv"
    officer_csv = sys.argv[4] if len(sys.argv) > 4 else "data/p5_officer.csv"
    activity_csv = sys.argv[5] if len(sys.argv) > 5 else "data/p5_activity.csv"

    print(f"Parsing {input_file}...")

    with gzip.open(input_file, 'rb') as f, \
         open(org_csv, 'w', newline='') as org_out, \
