...
def main():
    ...
    root_csv = sys.argv[2] if len(sys.argv) > 2 else "root.csv"
    info_csv = sys.argv[3] if len(sys.argv) > 3 else "info.csv"
    gpn_csv = sys.argv[4] if len(sys.argv) > 4 else "gpn.csv"
    lease_name_csv = sys.argv[5] if len(sys.argv) > 5 else "lease_name.csv"
